...
window.PatchSelectWidget = class PatchSelectWidget extends OO.ui.TagMultiselectWidget {
	constructor( config ) {
		super( $.extend( {
			allowArbitrary: true,
	...
	onInputKeyPress( e ) {
		...
		return super.onInputKeyPress( ...arguments );
	}

	createTagItemWidget() {
		const patchCache = this.constructor.static.patchCache,
			item = super.createTagItemWidget( ...arguments ),
	...
	doInputBackspace( e, withMetaKey ) {
		if (
...
OO.initClass( window.PatchSelectWidget );
